...
# Copied from https://github.com/P3rf/rofi-network-manager
# Added the functionallity to change the theme easily and a Network Editor GUI shortcut
...
function rofi_menu() {
	if [[ $(nmcli device | awk '$2=="wifi" {print $1}' | wc -l) -ne "1" ]]; then
		...
		-a "0" -lines "$LINES" -width -"$WIDTH" -theme "cybernight-network" \
...
function change_wireless_interface() {
	if [[ $(nmcli device | awk '$2=="wifi" {print $1}' | wc -l) -eq "2" ]]; then
		...
		rofi -dmenu -location "$LOCATION" -yoffset "$Y_AXIS" -xoffset "$X_AXIS" -font "$FONT"\
		-a "0" -lines "$LINES" -theme "cybernight-network" -width -16\
		-p ">_")
		for i in "${!WIRELESS_INTERFACES[@]}"
		do
...
function ssid_manual() {
	...
	# rofi -dmenu -location "$LOCATION" -yoffset "$Y_AXIS" -xoffset "$X_AXIS" -font "$FONT" \
	# -a "0" -lines 1 -width 18 -theme "cybernight-network" \
	# -p ">_")
	# echo $SSID
	# if [[ ! -z $SSID ]]; then
	# 	PASS=$(echo "Enter Password" | \
	# 	rofi -dmenu -location "$LOCATION" -yoffset "$Y_AXIS" -xoffset "$X_AXIS" -font "$FONT" \
	# 	-a "0" -lines 1 -width 18 -password -theme "cybernight-network" \
	# 	-p ">_")
	# 	if [ "$PASS" = "" ]; then
	# 			check_wifi_connected
...
function status() {
	...
	echo -e "$ETH_STATUS\n${WLAN_STATUS[@]}\n"| \
	rofi -dmenu -location "$LOCATION" -yoffset "$Y_AXIS" -xoffset "$X_AXIS" -font "$FONT" \
	-lines "$LINES" -width -"$WIDTH" -the "cybernight-network" \
	-p "Status"
}
			...
			if [[ ! -z "$SELECTION" ]] && [[ "$WIFI_LIST" =~ .*"$SELECTION".*  ]]; then
				...
				if [[ "$ACTIVE_SSID" == "$SSID_SELECTION" ]]; then
					...
					if [[ "$SELECTION" =~ "WPA2" ]] || [[ "$SELECTION" =~ "WEP" ]]; then
						...
						rofi -dmenu -location "$LOCATION" -yoffset "$Y_AXIS" -xoffset "$X_AXIS" -font "$FONT" \
						-a "0" -lines 1  -width 16 -password -theme "cybernight-network" \
						-p ">_")
					fi
					if [[ ! -z "$PASS" ]] ; then
